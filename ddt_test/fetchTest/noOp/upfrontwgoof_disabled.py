...
t0 = pd.read_csv('ufwg_t0_disabled.txt', sep=' ', names=col).iloc[:, -1]
t1 = pd.read_csv('ufwg_t1_disabled.txt', sep=' ', names=col).iloc[:, -1]
t2 = pd.read_csv('ufwg_t2_disabled.txt', sep=' ', names=col).iloc[:, -1]
...
fig.suptitle('prefetch upfront with calculation hardware prefetch disabled 2nd try')
