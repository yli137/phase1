...
t0 = pd.read_csv('upfrontsleep_t0.txt', sep=' ', names=col).iloc[:, -1]
t1 = pd.read_csv('upfrontsleep_t1.txt', sep=' ', names=col).iloc[:, -1]
t2 = pd.read_csv('upfrontsleep_t2.txt', sep=' ', names=col).iloc[:, -1]
...
fig.suptitle('prefetch upfront sleep for 10 micro seconds')
